      ...
      >
        ...
        {activeHotspots.map((item) => (
          ...
          >
            ...
            >
              <div className={cn("w-3 h-3 rounded-full bg-white transition-all duration-300", activeHotspotId === item.id ? "scale-[0.67]" : "scale-100")} />
              <div className={cn("absolute inset-0 rounded-full border-white transition-all duration-300", activeHotspotId === item.id ? "opacity-100 border-[1px]" : "opacity-0 border-0")} />
            </button>
          </div>
        ))}
