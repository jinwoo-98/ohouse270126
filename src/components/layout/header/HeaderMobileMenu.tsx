        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          {isLoading ? (
             <div className="space-y-4 p-4">
                {[1,2,3,4,5].map(i => <div key={i} className="h-10 bg-secondary animate-pulse rounded-xl" />)}
             </div>
          ) : (
                ...
                <motion.div key="main" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                  ...
                  <nav className="space-y-1">
                    {mainCategories.map((item) => (
                      <button 
                        key={item.name} 
                        onClick={() => item.hasDropdown && item.dropdownKey ? setActiveSubMenu(item.dropdownKey) : (navigate(item.href), onClose())} 
