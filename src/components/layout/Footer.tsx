...
export function Footer() {
  ...
  return (
    <footer className="bg-charcoal text-cream overflow-hidden relative border-t border-white/5">
      {/* 1. Thanh Đăng ký nhận tin */}
      <div className="bg-white/5 border-b border-white/5 py-10 md:py-14">
        <div className="container-luxury">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="text-center lg:text-left space-y-2">
              <h4 className="text-primary font-bold uppercase tracking-[0.2em] text-sm">Đăng ký nhận bản tin OHOUSE</h4>
              <p className="text-taupe text-xs font-medium">Nhận ngay voucher giảm 500K cho đơn hàng đầu tiên và cập nhật xu hướng mới nhất.</p>
            </div>
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row w-full lg:max-w-2xl gap-3">
              <div className="relative flex-1">
                <Input 
                  ...
                  required
                  className="bg-white/10 border-white/10 text-cream h-12 rounded-xl focus:ring-primary pr-12 placeholder:text-taupe/50"
                />
                <Mail className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-taupe/40" />
              </div>
              <Button 
                type="submit" 
                disabled={isLoading}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="space-y-4">
              ...
              <p className="text-taupe leading-relaxed text-sm font-medium">
            ...
            <div className="space-y-5 pt-2">
              <div className="flex items-start gap-3 group">
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-taupe font-bold mb-0.5">Hotline</p>
                  <p className="text-white font-bold text-sm">{settings?.phone || "1900 123 456"}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 group">
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-taupe font-bold mb-0.5">Email</p>
                  <p className="text-white font-bold text-sm">{settings?.email || "contact@ohouse.vn"}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 group">
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-taupe font-bold mb-0.5">Địa chỉ</p>
                  <p className="text-white font-bold text-sm leading-relaxed">{settings?.address || "Quận 1, TP. Hồ Chí Minh"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              {settings?.facebook_url && (
                <a href={sanitizeUrl(settings.facebook_url)} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 border border-white/10">
                  <Facebook className="w-4 h-4" />
                </a>
              )}
              {settings?.youtube_url && (
                <a href={sanitizeUrl(settings.youtube_url)} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 border border-white/10">
                  <Youtube className="w-4 h-4" />
                </a>
              )}
              {settings?.tiktok_url && (
                <a href={sanitizeUrl(settings.tiktok_url)} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 border border-white/10">
            ...
            <ul className="space-y-4">
              {isLoadingCategories ? (
                ...
                <>
                  ...
                  <li>
                    <Link to="/ho-tro/bao-mat" className="text-taupe hover:text-primary text-sm transition-colors flex items-center group">
                      <ArrowRight className="w-3 h-3 mr-2 opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300" />
                      Chính sách bảo mật
                    </Link>
                  </li>
                  <li>
                    <Link to="/ho-tro/dieu-khoan" className="text-taupe hover:text-primary text-sm transition-colors flex items-center group">
                      <ArrowRight className="w-3 h-3 mr-2 opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300" />
                      Điều khoản sử dụng
                    </Link>
                  </li>
          ...
          <div className="space-y-8">
            <div className="bg-white/5 p-6 rounded-2xl border border-white/5">
              <h4 className="text-primary font-bold uppercase tracking-widest text-[10px] mb-4">Cam kết dịch vụ</h4>
              <ul className="space-y-3">
                <li className="flex items-center gap-2 text-xs text-taupe">
            ...
            <AccordionItem value="support" className="border-white/5">
              ...
              <AccordionContent>
                <ul className="space-y-4 pb-4">
                  ...
                  <li><Link to="/ho-tro/bao-mat" className="text-taupe block text-sm">Chính sách bảo mật</Link></li>
                  <li><Link to="/ho-tro/dieu-khoan" className="text-taupe block text-sm">Điều khoản sử dụng</Link></li>
                </ul>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
