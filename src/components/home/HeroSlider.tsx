...
export function HeroSlider() {
  ...
  return (
    <section className="relative h-[60vh] md:h-[85vh] overflow-hidden bg-charcoal">
            ...
            >
              ...
              {slides[currentSlide].cta_text && (
                <Button
                  ...
                  asChild
                >
                  <a href={slides[currentSlide].cta_link}>
                    {slides[currentSlide].cta_text}
                  </a>
                </Button>
              )}
      ...
      {slides.length > 1 && (
        <>
          <button onClick={goToPrev} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 w-12 h-40 bg-card/20 backdrop-blur-md rounded-r-2xl text-cream hover:bg-primary hover:text-primary-foreground transition-all duration-500 z-20 items-center justify-center group">
            <ChevronLeft className="w-6 h-6 group-hover:-translate-x-1 transition-transform" />
          </button>
          <button onClick={goToNext} className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 w-12 h-40 bg-card/20 backdrop-blur-md rounded-l-2xl text-cream hover:bg-primary hover:text-primary-foreground transition-all duration-500 z-20 items-center justify-center group">
          ...
          <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-3">
            {slides.map((_, index) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${index === currentSlide ? "bg-primary w-8 md:w-12" : "bg-cream/30 w-2 md:w-3 hover:bg-cream/60"}`}
              />
            ))}
          </div>
        </>
